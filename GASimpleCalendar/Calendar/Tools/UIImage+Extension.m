...
+ (UIImage *)composeNewImageWithImages:(NSArray<UIImage *> *)images {
    CGSize size = CGSizeMake(images[0].size.width, 0);
    ...
    for (NSInteger i = 0; i < images.count; i ++) {
        if (i > 0) {
            height += images[i - 1].size.height;
        }
        [images[i] drawInRect:CGRectMake(0, height, images[i].size.width, images[i].size.height)];
    }
    UIImage *newImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    // 保存到相册中
